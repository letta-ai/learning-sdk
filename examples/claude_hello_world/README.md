# Claude Agent SDK Hello World with Agentic Learning

A simple example demonstrating how to use the Claude Agent SDK with the **Agentic Learning SDK** to create autonomous agents with persistent memory.

## Overview

This example shows how to add persistent memory to Claude Agent SDK applications using the Agentic Learning SDK. The agent will remember past conversations across sessions, making it truly stateful.

**What's Different:**
- ✅ Agent conversations are automatically captured and saved
- ✅ Relevant context from past interactions is retrieved and used
- ✅ Memory persists across restarts - the agent remembers previous sessions
- ✅ Zero architectural changes - just wrap your `query()` call with `learning()`

## Installation

```bash
npm install @anthropic-ai/claude-agent-sdk @letta-ai/agentic-learning typescript @types/node tsx zod
```
...
await learning({ agent: 'claude-hello-world-demo' }, async () => {
  ...
  for await (const message of q) {
    // Process messages - they're automatically captured!
  }
});
```

**That's it!** The agent now has persistent memory across sessions.

### Basic Structure (Without Memory)

For reference, the original Claude Agent SDK structure looks like this:
...
Once you've wrapped your agent with `learning()`, you get:

- **Automatic Capture**: All conversations are saved without manual intervention
- **Semantic Search**: Query past conversations using natural language
- **Context Injection**: Relevant memories are automatically added to prompts
- **Cross-Session Memory**: Agent remembers across restarts and deployments

### Example: Querying Agent Memory
...
const messages = await learningClient.memory.search({
  agent: 'claude-hello-world-demo',
  query: 'What did we discuss about TypeScript?'
});
```
...
- [Examples](https://github.com/letta-ai/agentic-learning-sdk/tree/main/examples)
